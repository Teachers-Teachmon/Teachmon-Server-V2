...
@DisplayName("조퇴 스케줄 설정 전략 테스트")
class AwayScheduleSettingStrategyTest {
    ...
    @InjectMocks
    private AwayScheduleSettingStrategy strategy;

    @Test
    @DisplayName("전략의 스케줄 타입은 AWAY여야 한다")
    void shouldReturnAwayScheduleType() {
        // When: 스케줄 타입을 가져오면
        ScheduleType scheduleType = strategy.getScheduleType();

        // Then: AWAY여야 한다
        assertThat(scheduleType).isEqualTo(ScheduleType.AWAY);
    }

    @Test
    @DisplayName("baseDate 이후의 조퇴 레코드에 대한 스케줄을 설정할 수 있다")
    void shouldSettingAwayScheduleForExistingRecords() {
        // Given: baseDate 이후에 조퇴 레코드가 있고 학생 스케줄도 존재할 때
        ...
        AwayEntity away = createMockAway(1L, student, baseDate, SchoolPeriod.SEVEN_PERIOD);
        StudentScheduleEntity studentSchedule = createMockStudentSchedule(1L, student, baseDate, SchoolPeriod.SEVEN_PERIOD);

        given(awayRepository.findAllFromDate(baseDate)).willReturn(List.of(away));
        ...
        // Then: 조퇴 스케줄이 1개 생성되어야 한다
        verify(awayScheduleRepository, times(1)).save(any(AwayScheduleEntity.class));
    }

    @Test
    @DisplayName("조퇴 레코드가 없으면 아무것도 생성하지 않는다")
    void shouldNotCreateSchedulesWhenNoAwayRecords() {
        // Given: baseDate 이후에 조퇴 레코드가 없을 때
        LocalDate baseDate = LocalDate.now().plusWeeks(1).with(java.time.DayOfWeek.MONDAY);

        given(awayRepository.findAllFromDate(baseDate)).willReturn(List.of());

        // When: 스케줄을 설정하면
        strategy.settingSchedule(baseDate);

        // Then: 아무것도 생성되지 않아야 한다
        verify(awayScheduleRepository, never()).save(any());
        verify(scheduleRepository, never()).save(any());
    }

    @Test
    @DisplayName("조퇴 레코드에 해당하는 학생 스케줄이 없으면 건너뛴다")
    void shouldSkipWhenStudentScheduleNotFound() {
        // Given: 조퇴 레코드는 있지만 해당 학생의 학생 스케줄이 없을 때
        LocalDate baseDate = LocalDate.now().plusWeeks(1).with(java.time.DayOfWeek.MONDAY);
        ...
        // Then: 조퇴 스케줄이 생성되지 않아야 한다
        verify(awayScheduleRepository, never()).save(any());
        verify(scheduleRepository, never()).save(any());
    }

    @Test
    @DisplayName("여러 조퇴 레코드가 있을 때 각각 독립적으로 처리된다")
    void shouldHandleMultipleAwayRecordsIndependently() {
        // Given: baseDate 이후에 2개의 조퇴 레코드가 있을 때
        LocalDate baseDate = LocalDate.now().plusWeeks(1).with(java.time.DayOfWeek.MONDAY);
        ...
        AwayEntity away1 = createMockAway(1L, student1, baseDate, SchoolPeriod.SEVEN_PERIOD);
        AwayEntity away2 = createMockAway(2L, student2, nextTuesday, SchoolPeriod.SEVEN_PERIOD);
        ...
        given(awayRepository.findAllFromDate(baseDate)).willReturn(List.of(away1, away2));
        ...
        // Then: 조퇴 스케줄이 2개 생성되어야 한다
        verify(awayScheduleRepository, times(2)).save(any(AwayScheduleEntity.class));
    }

    @Test
    @DisplayName("생성된 조퇴 스케줄은 기존 조퇴 레코드를 참조해야 한다")
    void shouldReferenceExistingAwayRecord() {
        // Given: 기존 조퇴 레코드가 있을 때
        LocalDate baseDate = LocalDate.now().plusWeeks(1).with(java.time.DayOfWeek.MONDAY);
        ...
        // Then: 저장된 조퇴 스케줄이 기존 조퇴 레코드를 참조해야 한다
        org.mockito.ArgumentCaptor<AwayScheduleEntity> captor =
                org.mockito.ArgumentCaptor.forClass(AwayScheduleEntity.class);
        verify(awayScheduleRepository).save(captor.capture());

        assertThat(captor.getValue().getAway()).isEqualTo(away);
    }
    ...
    private AwayEntity createMockAway(Long id, StudentEntity student, LocalDate day, SchoolPeriod period) {
        AwayEntity away = mock(AwayEntity.class);
        given(away.getId()).willReturn(id);
        given(away.getStudent()).willReturn(student);
        given(away.getDay()).willReturn(day);
        given(away.getPeriod()).willReturn(period);
        return away;
    }
